...
{
   if( (pFH->fFileVersionNumber < ABF_CURRENTVERSION) ||
...

   {
      ...
      // 4. New entries to accommodate 2 conditioning trains, user lists and P/N leak subtractions.
      ...
      if( uDAC == (UINT)pFH->nActiveDACChannel )
      {
         ...
         pFH->nPNADCNum[uDAC]            = pFH->_nPNADCNum;
   ...
   {
      ERRORMSG( "Error - OLDH_ABFtoCurrentVersion called with wrong file version !" );
   }
}
...
//          ADC channel #1
//    35  35 bytes of environment data
//    64  64-byte ASCII character string for the conditioning pulse parameters
...
//===============================================================================================
// FUNCTION: ReadCondit
// PURPOSE:  Read conditioning train parameters from an old file.
//   
static BOOL ReadCondit(HANDLE hFile, char *Condit)
{
   // Seek to the conditioning string
   SetFilePointer(hFile, 512L, NULL, FILE_BEGIN);

   // Read the conditioning pulse string.
                             ...
                             char *Condit, char *Comment, ABFFileHeader *pFH )
{
   ...
   // GROUP #9 (32 bytes) - Conditioning pulse train.
   ...
   if (Param[C52_AUTOPEAKSEARCHMODE] == 0)
      pFH->nAutopeakEnable = 0;
      pFH->nAutopeakEnable = 1;
   pFH->nAutopeakADCNum = short(Param[C52_AUTOPEAKCHANNEL]);

   n = short(Param[C52_AUTOPEAKSEARCHMODE]);
   switch (n)
   {
      case 5:
         pFH->nAutopeakSearchMode = -1;
         break;
      case 6:
         pFH->nAutopeakSearchMode = 0;
         break;
      default:
         pFH->nAutopeakSearchMode = short(Param[C52_AUTOPEAKSEARCHMODE]);
         break;
   }
   ...
   if (Param[C52_AUTOPEAKCENTER] < 0)
      ...
      pFH->nAutopeakPolarity = 1;
   pFH->lAutopeakStart     = 0;
   pFH->lAutopeakEnd       = 0;
   pFH->nAutopeakBaseline  = short(Param[C52_BASELINECALCULATION]);
   pFH->nAutopeakSmoothing = short(Param[C52_AUTOPEAKAVPOINTS]);
   if (pFH->nAutopeakSmoothing < 1)
      pFH->nAutopeakSmoothing = 1;
   pFH->nAutopeakAverage   = 0;
