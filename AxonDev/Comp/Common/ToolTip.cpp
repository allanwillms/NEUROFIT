...
//
void CTooltip::FillInToolInfo(CToolInfo &ti, HWND hWnd) const
{
...
// PURPOSE:  Helper function to fill in a TOOLINFO structure corectly.
//
void CTooltip::FillInToolInfo(CToolInfo &ti, HWND hwndParent, UINT uIDTool, LPCRECT pRect) const
{
   ti.hwnd   = hwndParent;
   ti.uId    = uIDTool;
...
void CTooltip::GetText(HWND hWnd, LPSTR pszText, UINT uTextLen) const
{
   ...
   AXU_strncpyz(pszText, ToolInfo.szText, uTextLen);
}
...
//
BOOL CTooltip::GetToolInfo(HWND hWnd, CToolInfo& ToolInfo) const
{
   MEMBERASSERT();
   HWNDASSERT(m_hWnd);
...
// PURPOSE:  Gets the full TOOLINFO associated with a tool.
//
BOOL CTooltip::GetToolInfo(HWND hwndParent, UINT nIDTool, CToolInfo& ToolInfo) const
{
   MEMBERASSERT();
   HWNDASSERT(m_hWnd);
...
//
BOOL CTooltip::GetToolInfoByIndex(UINT uToolInx, CToolInfo& ToolInfo) const
{
   MEMBERASSERT();
   HWNDASSERT(m_hWnd);
